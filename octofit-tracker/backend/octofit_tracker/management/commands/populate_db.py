...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        # Clear existing data
        if User.objects.exists() is not None:
            User.objects.all().delete()
        if Team.objects.exists() is not None:
            Team.objects.all().delete()
        if Activity.objects.exists() is not None:
            Activity.objects.all().delete()
        if Leaderboard.objects.exists() is not None:
            Leaderboard.objects.all().delete()
        if Workout.objects.exists() is not None:
